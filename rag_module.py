"""
动态RAG模块 - 基于Kimi API联网搜索的学术文献检索（四阶段辩论版）
...
RAG_CONFIG = {
    ...
    # 专家缓存过期时间（小时）- 四阶段辩论期间缓存有效
    "agent_cache_duration_hours": 12,
    # Kimi API配置（使用联网搜索）
...
class RAGCache:
    ...
    def _get_agent_cache_key(self, agent_role: str, debate_topic: str) -> str:
        """生成专家角色特定的缓存键（四阶段辩论版）"""
        try:
            key_string = f"debate_agent_{agent_role}_{debate_topic}"
            return hashlib.md5(key_string.encode()).hexdigest()
        except Exception as e:
    ...
    def cache_agent_context(self, agent_role: str, debate_topic: str, context: str):
        ...
        try:
            ...
            cache_data = {
                ...
                'context': context,
                'debate_version': 'four_stage_v1'  # 四阶段版本标识
            }
            
            with open(cache_file, 'w', encoding='utf-8') as f:
                json.dump(cache_data, f, ensure_ascii=False, indent=2)
                
            print(f"✅ 已缓存专家 {agent_role} 的四阶段辩论学术资料")
        ...
        try:
            if agent_role:
                ...
                for filename in os.listdir(self.agent_cache_dir):
                    if filename.startswith(f"debate_agent_{agent_role}_"):
                        try:
...
class WebSearchTool:
    """基于Kimi API的$web_search工具实现 (集成JSON Mode，四阶段辩论优化)"""
    ...
    def __init__(self, api_key: str = None):
        ...
        if not self.api_key:
            ...
            print("✅ Kimi联网搜索工具初始化成功（四阶段辩论版）")
    
    def web_search_impl(self, arguments: Dict[str, Any]) -> Any:
        """实现web_search工具的具体逻辑"""
        return arguments
    
    def search_with_web_tool(self, query: str, agent_role: str = "", debate_stage: str = "opening") -> str:
        """使用Kimi的$web_search工具进行联网搜索 (启用JSON Mode，四阶段辩论优化)"""
        if not self.api_key:
            print("❌ Kimi API Key 未配置")
            return "联网搜索功能不可用，API密钥未设置。"
        
        try:
            # 构建搜索提示词 (JSON Mode，四阶段辩论优化)
            search_prompt = self._build_web_search_prompt_json(query, agent_role, debate_stage)
            
            print(f"🔍 正在使用Kimi联网搜索 (四阶段辩论JSON Mode): {query}")
    ...
    def _build_web_search_prompt_json(self, query: str, agent_role: str = "", debate_stage: str = "opening") -> str:
        """构建使用JSON Mode的联网搜索提示词（四阶段辩论优化）"""
        role_context = ""
        stage_context = ""
        ...
        if debate_stage:
            stage_mapping = {
                "opening": "为开辩综述提供全面的基础资料",
                "questioning": "为提问和回答环节提供深度分析资料",
                "free_debate": "为自由辩论提供有力的论证支撑",
                "closing": "为结辩总结提供前瞻性的资料"
            }
            stage_context = f"，{stage_mapping.get(debate_stage, '为四阶段辩论提供支撑')}"
        
        prompt = f"""请使用联网搜索功能，{role_context}帮我搜索关于"{query}"的最新信息和学术资料{stage_context}。
...
现在请为我搜索关于"{query}"的信息并以JSON格式返回："""
    ...
    def _call_kimi_with_web_search_json(self, prompt: str) -> Optional[str]:
        """调用Kimi API并支持$web_search工具和JSON Mode（四阶段辩论优化）"""
        try:
            headers = {
            ...
            # 初始请求消息
            messages = [
                {"role": "system", "content": "你是Kimi。由Moonshot AI提供的人工智能助手，你更擅长中文和英文的对话。你会为用户提供安全，有帮助，准确的回答。同时，你会寻求回应用户使用JSON格式的要求，如用户要求JSON格式输出。专门为四阶段辩论系统提供优质的学术资料搜索服务。"},
...
class AcademicSearcher:
    """基于Kimi API联网搜索的学术文献检索器 (集成JSON Mode，四阶段辩论优化)"""
    ...
    def __init__(self, api_key: str = None):
        ...
        if not self.web_tool.api_key:
            ...
            print("✅ 学术联网搜索器初始化成功（四阶段辩论版）")
    
    def search(self, query: str, max_results: int = 5, agent_role: str = "", debate_stage: str = "opening") -> List[SearchResult]:
        """使用Kimi联网搜索进行学术文献检索 (JSON Mode，四阶段辩论优化)"""
        try:
            print(f"🔍 正在使用Kimi联网搜索学术文献 (四阶段辩论JSON Mode): {query} (最多{max_results}篇)")
            
            # 使用联网搜索 (JSON Mode，四阶段辩论优化)
            search_response = self.web_tool.search_with_web_tool(query, agent_role, debate_stage)
    ...
    def _parse_json_search_response(self, response: str, query: str, max_results: int) -> List[SearchResult]:
        ...
        try:
            ...
            print(f"✅ 四阶段辩论JSON联网搜索解析得到 {len(results)} 篇文献")
            return results
...
class DynamicRAGModule:
    """动态RAG主模块（基于Kimi API联网搜索的学术文献检索，四阶段辩论版）"""
    ...
    def search_academic_sources(self, 
                              ...
                              agent_role: str = "",
                              debate_stage: str = "opening") -> List[SearchResult]:
        """
        搜索学术数据源（使用联网搜索，集成JSON Mode，四阶段辩论优化）
        """
        
        if max_results_per_source is None:
            max_results_per_source = RAG_CONFIG["max_results_per_source"]
        
        print(f"🔍 四阶段辩论JSON Mode联网学术搜索配置：最多{max_results_per_source}篇，角色定制：{agent_role}，阶段：{debate_stage}")
        ...
        # 联网搜索 (JSON Mode，四阶段辩论优化)
        if "web_search" in sources or "kimi" in sources:
            try:
                search_results = self.academic_searcher.search(topic, max_results_per_source, agent_role, debate_stage)
                all_results.extend(search_results)
                print(f"🌐 四阶段辩论JSON Mode联网搜索找到 {len(search_results)} 篇文献")
                
            except Exception as e:
                print(f"❌ 四阶段辩论JSON Mode联网搜索出错: {e}")
    ...
    def get_rag_context_for_agent(self, 
                                 ...
                                 debate_stage: str = "opening") -> str:
        """
        为特定角色获取基于联网搜索的RAG上下文 (JSON Mode，四阶段辩论优化)
        """
        
        print(f"🔍 为专家{agent_role}四阶段辩论JSON Mode联网搜索学术资料，最大文献数{max_sources}篇，阶段：{debate_stage}")
        ...
        # 使用用户设置的数量进行联网搜索 (JSON Mode，四阶段辩论优化)
        try:
            ...
            print(f"❌ 四阶段辩论JSON Mode联网搜索失败: {e}")
        ...
        if not results:
            ...
            try:
                ...
                print(f"📊 四阶段辩论JSON Mode联网搜索结果处理：为专家 {agent_role} 实际搜索到 {len(results)} 篇，按用户设置选择前 {len(top_results)} 篇")
...
"""
                ...
                print(f"✅ 四阶段辩论JSON Mode联网搜索上下文构建完成：{final_ref_count}篇参考文献")
    ...
    def _create_role_focused_query(self, agent_role: str, debate_topic: str) -> str:
        ...
        try:
            ...
            print(f"🎯 为{agent_role}定制四阶段辩论JSON Mode联网搜索查询：{focused_query}")
    ...
    def clear_all_caches(self):
        ...
        try:
            ...
            for filename in os.listdir(self.cache.cache_dir):
                if filename.endswith('.json') and not filename.startswith('debate_agent_'):
                    try:
...
def initialize_rag_module(llm: ChatDeepSeek) -> DynamicRAGModule:
    """初始化RAG模块（基于Kimi API联网搜索，四阶段辩论版）"""
    global rag_module
    try:
        rag_module = DynamicRAGModule(llm)
        print("🔍 四阶段辩论RAG模块已初始化，使用Kimi API联网搜索功能")
        return rag_module
    except Exception as e:
        print(f"❌ 四阶段辩论RAG模块初始化失败: {e}")
        return None
...
# 四阶段辩论版测试函数
def test_four_stage_rag_module():
    """测试基于Kimi JSON Mode联网搜索的RAG模块功能（四阶段辩论版）"""
    print("🧪 开始测试基于Kimi JSON Mode联网搜索的RAG模块（四阶段辩论版）...")
    ...
    try:
        ...
        for stage in ["opening", "questioning", "free_debate", "closing"]:
            print(f"🔍 测试{stage}阶段专家角色文献检索：{test_role}")
            try:
                ...
                if context and context != "暂无相关学术资料。":
                    ...
                    print(f"✅ {stage}阶段测试成功：获得{ref_count}篇文献")
                    print(f"前100字符：{context[:100]}...")
                else:
                    print(f"⚠️ {stage}阶段未找到学术资料")
            except Exception as e:
                print(f"❌ {stage}阶段测试出错: {e}")
            
            print("-" * 50)
            
    except Exception as e:
        print(f"❌ 四阶段辩论RAG模块测试失败: {e}")

if __name__ == "__main__":
    test_four_stage_rag_module()